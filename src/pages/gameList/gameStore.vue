  ...
  <el-card shadow="never" class="border-0">
    ...
    <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="status" label="状态">
        <template #default="{ row }">
          <el-switch :model-value="row.status" :active-value="1" :inactive-value="0" @click="handleStoreStatus(row)"/>
...
import {queryGameStore, createGameStore, updateGameStore, deleteGameStore, batchUpdateSort,updateGameStoreStatus} from "~/api/gameStore"
...
const rules = {
  ...
  cateType: [
    {type: 'number', message: 'cateType使用数字表示'}
  ],
  tagType: [
    {type: 'number', message: 'tag使用数字表示'}
  ],
}
...
const handleSubmit = () => {
  formRef.value.validate((valid) => {
    ...
    const fun = editId.value ? updateGameStore(form) : createGameStore(form)
...
const handleStoreStatus=(row)=>{
  ...
  //变更游戏状态
  updateGameStoreStatus(data).then(res => {
    toast("修改状态成功！")
    getData()
  }).finally(() => {
    loading.value = false
