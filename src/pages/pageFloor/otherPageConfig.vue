<template>
  <el-card shadow="never" class="border-0">
    ...
    <el-table :data="tableData" style="width: 100%" v-loading="loading" calss="el-table__fixed-body-wrapper"
      ...
      <el-table-column prop="title" label="floor名称" width="150"/>

      <el-table-column prop="description" label="描述"/>

      <el-table-column prop="status" label="状态" width="115px">
        <template #default="{ row }">
          <el-switch :model-value="row.status" :active-value="1" :inactive-value="0"
                     @click="handlePageFloorStatus(row)"/>
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template #default="scope">
          ...
          <el-button type="primary" size="small" text @click="handleGroupList(scope.row)" style="margin-left: -10px;">
            分组列表
          </el-button>
    ...
    <FromDrawer :title="drawerTitle" ref="formDrawerRef" @cancel="handleCancel" @submit="handleSubmit" size="500px"
                destroy-on-close>
      <el-form :model="linkForm" ref="formRef" :rules="rules" label-width="80px" :inline="false">
        <el-form-item label="名称" prop="title">
          <el-input v-model="linkForm.title" placeholder="请输入分组标题"></el-input>
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input v-model="linkForm.description" placeholder="请输入描述" type="textarea" :rows="5"></el-input>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select v-model="linkForm.status" placeholder="选择状态" clearable>
...
const handleGroupList = (row) => {
  loading.value = true
  pageFloorId.value = row.id
...
//修改状态 status
const handlePageFloorStatus = (row) => {
  loading.value = true
  let data = {
    id: row.id,
    newStatus: 0
  ...
  //变更状态
  updatePageFloorStatus(data).then(res => {
    toast("修改状态成功！")
    getData()
  }).finally(() => {
    loading.value = false
